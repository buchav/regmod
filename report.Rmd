...
Later we studied relationship of MPG and other variables by building several regression models (MPG ~ Weight+Transmission) and (MPG ~ Weight*Transmission). Although the best linear regression model allows to predict ~83% of MPG variability, we can not use it to judge what transmission is better in respect to weight and transmission type.The main reason is that mtcars dataset have very small overlap between the trasmission groups for any given weight. 
